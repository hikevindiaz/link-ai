...
-- Create a centralized vector documents table
-- This approach is more scalable than per-knowledge-source tables
...
-- Add columns to existing tables for Supabase vector integration
...
-- Add vector document IDs to content tables for tracking
ALTER TABLE public."text_contents"
  ADD COLUMN IF NOT EXISTS vector_document_id UUID REFERENCES public.vector_documents(id);
