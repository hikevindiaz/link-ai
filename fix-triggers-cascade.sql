...
-- Handle existing triggers properly with CASCADE
...
-- Drop existing file triggers with CASCADE to handle dependencies
...
-- Fix text content triggers (check for existing names first)
...
RAISE NOTICE 'Deleted % failed file jobs with non-existent content_ids', ROW_COUNT();
...
RAISE NOTICE 'Created % new embedding jobs for existing files', ROW_COUNT();
