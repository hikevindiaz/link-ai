...
BEGIN
  ...
  -- Queue these jobs with correct pgmq.send signature
  FOR job_record IN 
  ...
  LOOP
    ...
    BEGIN
      ...
      SELECT pgmq.send('embedding_queue', jsonb_build_object('job_id', job_record.job_id)) INTO queue_result;
      job_count := job_count + 1;
      RAISE NOTICE 'Queued job (jsonb): %', job_record.job_id;
    EXCEPTION WHEN OTHERS THEN
      BEGIN
        -- Try with text message
        SELECT pgmq.send('embedding_queue', job_record.job_id::text) INTO queue_result;
        job_count := job_count + 1;
      ...
      EXCEPTION WHEN OTHERS THEN
        BEGIN
          ...
          job_count := job_count + 1;
          RAISE NOTICE 'Queued job (explicit): %', job_record.job_id;
        EXCEPTION WHEN OTHERS THEN
